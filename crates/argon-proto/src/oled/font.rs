...
//! layout matches the SSD1306's page memory exactly, so drawing a character is a copy rather
//! than a transform.
...
/// Column data for 95 printable characters, 0x20 through 0x7E.
static FONT: [[u8; GLYPH_WIDTH]; 95] = [
...
pub fn glyph(c: char) -> &'static [u8; GLYPH_WIDTH] {
    ...
    let index = code.saturating_sub(FIRST) as usize;
    FONT.get(index).unwrap_or(&FONT[(b'?' - FIRST) as usize])
}
