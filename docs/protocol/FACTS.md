...
| `ARGON-UPS-CMD6-FW17` | **Firmware 17 accepts the wake-schedule write** (`CMD6`): after T17, argond read the schedule as exactly T17's second target (2097-03-21 17:42), where it had read none before. Whether the first write took is unconfirmed; whether a wake *fires* on this firmware (T18) is untested | `observed` | OBS-2026-09-23-neo5-industria-ups-5000 |
| `ARGON-UPS-WAKE-FW17` | **The wake fires on firmware 17**: T18 powered off a NEO 5 and its Industria UPS 5000 powered it on at the scheduled minute, 20 minutes later. Run with the Pi's EEPROM matched to the V5 (`POWER_OFF_ON_HALT=1`, `WAKE_ON_GPIO=0`), so the UPS firmware was the one variable. Whether it also works with `POWER_OFF_ON_HALT=0` is untested | `observed` | OBS-2026-09-23-neo5-industria-ups-5000 |
| `ARGON-UPS-WAKE-CLEAR-FW17` | On firmware 17, as on 113, **a fired schedule clears itself**: argond found none on its first check after the wake | `observed` | OBS-2026-09-23-neo5-industria-ups-5000 |
...
| `ONE-V3-MCU-REGISTER` | On an **Argon ONE V3** with a Pi 5, the MCU speaks the **register protocol**: register `0x80` holds the fan duty and is readable -- written `01`, it read back `01`. Observed on the bus from the vendor daemon (kernel I2C tracepoints), not from its code. A register read there changes nothing; on the ONE V1 it pins the fan at full | `observed` | OBS-2026-09-23-t5-one-v3-register-dialect |
| `ONE-V3-ARGOND-REGISTER` | With `[mcu] dialect = "register"`, **every write argon-utils makes to the ONE V3's MCU is a `[0x80, duty]` frame** -- signal handler, exit guard, stop hook and control loop alike, traced on the wire. None is a single byte | `observed` | OBS-2026-09-23-t5-one-v3-register-dialect |
| `ARGON-QUICKWRITE-NO-DATA` | The presence probe (`SMBus` quick-write) reaches the bus as a **zero-length write** (`l=0 []`): address and write bit, no data byte for any firmware to misread | `observed` | OBS-2026-09-23-t5-one-v3-register-dialect |
...
| `ONE-V3-BTN-DOUBLE-TAP` | On an **Argon ONE V3** with a Pi 5, a **double-tap** sends one pulse of **20.00 ms** (within 4 us across five) on GPIO4, active high | `observed` | OBS-2026-09-23-t3-one-v3-button |
| `ONE-V3-BTN-SINGLE-SILENT` | On the V3 a **single tap sends nothing**: no edge on any safe header line, no input event, no byte on UART0. Unlike the ONE V1, where a single tap pulses | `observed` | OBS-2026-09-23-t3-one-v3-button |
